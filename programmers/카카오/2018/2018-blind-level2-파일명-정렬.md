## 2018 카카오 블라인드 채용 - 파일명 정렬 [🔗](https://programmers.co.kr/learn/courses/30/lessons/17686)
...
파이썬 정렬을 그대로 사용해서 거의 날먹 문제였다.

해설을 보니 비교했을 때 동일한 경우 입력 순서를 유지하는 정렬을 stable sort라 한다고 한다. 운이 좋게도 python과 c++은 기본 정렬 함수가 안정 정렬이라고 한다.

```python
import re

def sortKey(fileName):
    return (fileName[0], int(fileName[1]))

def solution(files):
    p = re.compile('([^\d]+)([\d]{1,5})(.*)')
    fileNames = {fn: p.match(fn.lower()).groups() for fn in files}
    
    files.sort(key=lambda x: sortKey(fileNames[x]))
    return files
```
